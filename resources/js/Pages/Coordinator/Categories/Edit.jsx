// resources/js/Pages/Coordinator/Categories/Edit.jsx
import AuthenticatedLayout from '@/Layouts/AuthenticatedLayout';
import { Head, useForm, Link } from '@inertiajs/react';
import { useState } from 'react';

export default function Edit({ auth, category, allCategories }) {
    const { data, setData, put, processing, errors } = useForm({
        name: category.name || '',
        description: category.description || '',
        prerequisites: category.prerequisites?.map(p => p.id) || [],
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('coordinator.categories.update', category.id));
    };
                    ...
                    <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                        ...
                        <div className="bg-gradient-to-r from-green-600 to-green-700 px-8 py-6">
                            <div className="flex items-center justify-between">
                                <div>
                                    <h2 className="text-2xl font-bold text-white">Edit Kategori Pelatihan</h2>
                                    <p className="text-green-100 text-sm mt-1">
                                        Perbarui informasi kategori dan atur prerequisite jika diperlukan
                                    </p>
                                </div>
                                <Link
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-7">
                                        ...
                                        {filteredCategories.length > 0 ? (
                                            filteredCategories.map((cat) => (
                                                ...
                                                >
                                                    <input
                                                        type="checkbox"
                                                        checked={data.prerequisites.includes(cat.id)}
                                                        onChange={() => togglePrerequisite(cat.id)}
                            ...
                            <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-xl p-6">
                                <div className="flex items-start">
                                    ...
                                    <div className="ml-4">
                                        <p className="text-sm font-bold text-green-900">Tentang Prerequisite</p>
